...
def fetch_gsc_data(site_url: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    service = build("searchconsole", "v1", credentials=creds)  # google-api-python-clientでのGSCはbetaのものなど注意
    
    # ただしsearchconsole v1 は一部メソッドが非対応の場合あり。
    # 公式には webmasters / searchconsole の discoveryDocsが存在。ここではイメージサンプルに留めます。
    ...
    # 実際には webmasters().searchanalytics().query(siteUrl=site_url, body=request).execute()
    # などで行うが、searchconsole API clientの書き方が少し特殊なので詳細は要調整
...
def main_fetch_all():
    """
    まとめてデータを取得 → 必要に応じて結合 or 加工し、CSVなどに書き出す処理の例。
    GitHub Actionsのcronや手動トリガーで呼び出し、DBやどこかに保存しておくなど。
    """
    # 例1) GSCデータ
    site_url = "sc-domain:good-apps.jp"  # または "https://good-apps.jp/"
    gsc_df = fetch_gsc_data(site_url, "2025-01-01", "2025-01-31")  # 例
    
    # 例2) BigQueryデータ
    sql = "SELECT * FROM `your_project.your_dataset.your_table` LIMIT 100"
    project_id = "741349417998"  # Apps Scriptで使っていたプロジェクト番号を指定
    ...
    # ここでDataFrame同士をキー(URLなど)でjoinする・縦方向にconcatする等、必要に応じて加工
    # ...
    # 最終的にCSV保存する例:
    merged_df = gsc_df  # 仮のまま
